...
const createUserCart = async (email, _id, size, quantity) => {
  // OK!
  try {
    const existingCart = await User.findOne({ email, carts: { $elemMatch: { productId: new ObjectId(_id), size } } });
    ...
    if (existingCart) {
      await User.findOneAndUpdate(
        ...
        { new: true, arrayFilters: [{ 'item.productId': new ObjectId(_id), 'item.size': size }] }
      );
    } else {
...
const countSelectedUserCarts = async (email, productId, size) => {
  ...
  try {
    ...
    return (
      user.carts.find(cart => `${cart.productId}` === `${new ObjectId(productId)}` && cart.size === size)?.quantity || 0
    );
  } catch (err) {
